                            ...
                            {!viewMode && (
                                <div className="bg-gray-50/50 p-6 rounded-3xl flex justify-between items-center no-print mt-10">
                                    ...
                                    <button 
                                        onClick={async () => {
                                            const saved = await handleSave(true);
                                            // Make sure exportVerbaleToDocx exists or we do it inline.
...
const idxEndMatch = content.match(new RegExp("\\\\)\\\\\\}\\\\s*<\\\\/div>\\\\s*\\\\{\\\\/\\\\* SAVE MODAL \\\\*\\\\/"));

if (idxStart !== -1 && idxEndMatch) {
    const idxEnd = idxEndMatch.index;
    content = content.substring(0, idxStart) + anteprimaTableHtml + "\\n                    </div>\\n                " + content.substring(idxEnd);
} else {
    console.error("Marker not found!", idxStart !== -1, idxEndMatch != null);
}

fs.writeFileSync('src/pages/Verbali/VerbaleEditor.tsx', content, 'utf8');
console.log('Fixed exactly using reliable regex index slicing!');
